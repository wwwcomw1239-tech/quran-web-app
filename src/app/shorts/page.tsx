...
import { getVideos, ShortsVideo } from '@/lib/archiveFetch';
...
export default function ShortsPage() {
  ...
  const handleScroll = useCallback((e: React.WheelEvent) => {
    ...
    containerRef.current.querySelector(`[data-index="${newIndex}"]`)?.scrollIntoView({ behavior: 'smooth' });
  }, [activeIndex, videos.length]);

  // Keyboard navigation
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      const isDown = e.key === 'ArrowDown' || e.key === 'ArrowRight' || e.key === ' ';
    ...
    return (
      <div className="fixed inset-0 bg-black flex flex-col items-center justify-center z-50 px-6">
        ...
        <div className="flex gap-4">
          <button onClick={loadVideos} className="flex items-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-full px-6 py-3 font-medium">
            <RefreshCw className="w-5 h-5" />
            <span>إعادة المحاولة</span>
          </button>
          <Link href="/" className="flex items-center gap-2 bg-white/10 hover:bg-white/20 text-white rounded-full px-6 py-3 font-medium">
    ...
    <div className="fixed inset-0 bg-black z-40" dir="rtl">
      ...
      <div className="absolute top-0 left-0 right-0 z-50 px-4 py-3 bg-gradient-to-b from-black/80 to-transparent">
        <div className="flex items-center justify-between max-w-lg mx-auto">
          <Link href="/" className="w-10 h-10 rounded-full bg-white/10 backdrop-blur-sm flex items-center justify-center text-white hover:bg-white/20">
            <Home className="w-5 h-5" />
          </Link>
          ...
          <button onClick={loadVideos} className="w-10 h-10 rounded-full bg-white/10 backdrop-blur-sm flex items-center justify-center text-white hover:bg-white/20">
      ...
      {/* Video Feed */}
      <div ref={containerRef} className="h-[100dvh] w-full overflow-y-scroll snap-y snap-mandatory scrollbar-hide" style={{ scrollSnapType: 'y mandatory' }} onWheel={handleScroll}>
        {videos.map((video, index) => (
          <div key={`${video.id}-${index}`} data-video-item data-index={index} className="h-[100dvh] w-full snap-start snap-always relative" style={{ scrollSnapAlign: 'start' }}>
            <ShortsVideoPlayer video={video} isActive={index === activeIndex} onLike={handleLike} isLiked={likedVideos.has(video.id)} />
          </div>
        ))}
      </div>
      ...
      <div className="absolute bottom-4 left-1/2 transform -translate-x-1/2 z-50">
        <div className="bg-black/50 backdrop-blur-sm px-4 py-2 rounded-full border border-white/10">
          <span className="text-white/70 text-sm">{activeIndex + 1} / {videos.length}</span>
        </div>
      </div>
